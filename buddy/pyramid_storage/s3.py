...
class S3FileStorage(object):
    ...
    def from_settings(cls, settings, prefix):
        options = (
            ...
            #('aws.host', False, None),
            #s('aws.port', False, None),
            ('aws.region', False, 'us-east-1'),
            ('aws.timeout', False, 5),
            ('aws.endpoint_url', False,'https://s3.amazonaws.com')
        )
    ...
    def get_connection(self):
        ...
        options['endpoint_url'] = options.pop('endpoint_url')
        ...
        cors_configuration = {
            'CORSRules': [{
                ...
                'AllowedOrigins': ['https://nairabricks.com'],
                'ExposeHeaders': ['GET', 'PUT','DELETE',"POST"],
                'MaxAgeSeconds': 3000
            }]
        }
        ...
        client.put_bucket_cors(Bucket=self.bucket_name, CORSConfiguration=cors_configuration)
        return client

    def folders(self, prefix='', suffix='.jpg'):
        paginator = self.get_connection().get_paginator('list_objects')
        for result in paginator.paginate(Bucket=self.bucket_name, Prefix=prefix, Delimiter='/'):
            for prefix in result.get('CommonPrefixes', []):
                yield prefix.get('Prefix')

    def key_list(self, folder):
        return list(self.folders(prefix=folder))
    ...
    def save_file_io(self, file_like_object, filename, folder=None, randomize=False,
                  extensions=None, acl=None):
        acl = acl or self.acl
        extensions = extensions or self.extensions
        ...
        self.get_connection().put_object(Bucket=self.bucket_name,
                                         Key=filename,
                                         Body=file_like_object.getvalue(),
                                         Metadata=metadata,
                                         ContentType=content_type,
                                         ACL=acl)
        return filename
