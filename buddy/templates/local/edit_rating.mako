...
<div class="container">
    <div class="row">
        <div class="col-sm-12">
            <div class="wrapper">
            <h2 class="page-header">Editing Rating and review of ${locality.city_name.title()} in ${locality.state.name} <code>Ratings must not be less than 2 stars</code></h2>
                ...
                <div class="col-sm-4">
                    ...
                    %for child in ftype.children:
                        <li>${form.label(child.name)}:
                           ${form.text(child.name,class_="rating required",value=locality.ratingByFeature[child.name])}
                                            </li>
                    %endfor
                            </ul>
                    </div>
            ...
            ${form.text('title', value=rating.title,class_='form-control')}
            ${form.label('review', class_="control-label")}
            ${form.textarea('review',rows="5", content=rating.review,class_='form-control required')}<br>
